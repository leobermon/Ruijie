...
moment().tz("America/Cancun").format();
...
function getSyscom(pag) {
    
    return new Promise(resolve => {
        //obtenemos solo los productos de la marca ruijie
        axios.get('https://developers.syscom.mx/api/v1/marcas/ruijie/productos?pagina='+pag)
...
function insertProduct(p, bandId, category, supplier, date, getPrice) {
    ...
    //logeamos
    console.log(`INSERT INTO ps_product(id_supplier,id_manufacturer,id_category_default,id_tax_rules_group,supplier_reference,reference,mpn,cache_default_attribute,date_add,date_upd,price,unity,redirect_type, available_date,active) VALUES (' + supplier + ', ' + bandId + ', ' + category + ', ' + id_tax + ', "' + p.producto_id + '","' + p.modelo + '","' + p.modelo + '", 0, "' + date + '", "' + date + '", ' + getPrice + ', "", "301-category", "0000-00-00", 1)`)
...
function actualizarPrecioPorPorcentaje() {
    ...
    console.log('funcion actuializarPrecioPorPorcentaje')
    console.log(`UPDATE ps_productos_syscom SET precioMasPorcentaje = CASE WHEN precioFinal < 300 THEN (precioFinal) + (precioFinal* (SELECT porcentaje FROM ps_supplier_percentage WHERE id = 6 ) / 100.0) WHEN precioFinal > 300 AND precioPromocion > 0 THEN (precioPromocion) + ((precioPromocion) * porcentaje / 100.0) WHEN precioFinal > 300 AND precioFinal > 0 THEN (precioFinal) + ((precioFinal) * porcentaje / 100.0) WHEN porcentaje = 0 THEN precioFinal ELSE precioFinal END WHERE  id_product > 0`)
    return new Promise(resolve => {
        db.getConnection((err, conn) => {
            conn.query('UPDATE ps_productos_syscom SET precioMasPorcentaje = CASE WHEN precioFinal < 300 THEN (precioFinal) + (precioFinal* (SELECT porcentaje FROM ps_supplier_percentage WHERE id = 6 ) / 100.0) WHEN precioFinal > 300 AND precioPromocion > 0 THEN (precioPromocion) + ((precioPromocion) * porcentaje / 100.0) WHEN precioFinal > 300 AND precioFinal > 0 THEN (precioFinal) + ((precioFinal) * porcentaje / 100.0) WHEN porcentaje = 0 THEN precioFinal ELSE precioFinal END WHERE  id_product > 0 ', function (error, result) {
                if (error) resolve({ mensaje: 'fail', data: error });
                console.log('despues del actualizarPrecioPorPorcentaje');
                resolve({ mensaje: 'ok', data: '' })
                conn.release();
            });
        })
...
async function asyncCall() {
    ...
    if (obtuvimosTipoCambio) { // solo si obtenemos el tipo de cambioo 
        ...
        //consultamos el numero de paginas 
        var validator2 = await getSyscom(1)

        console.log('validator1111111111')
        console.log(validator2.totalPaginas)

        console.log('va a entrar al for primeritisimo')
        

    for(var a=1;a++;a<=validator2.totalPaginas){
        console.log('dentro del for  en la iteracion = ' + a)
        ...
        //pasamos el parametro de pagina
        var validator = await getSyscom(1) // obtener datos de la api de syscom, donde solo sea de la amrca ruijie
        console.log('Imprimimos cantPaginasProductos====')
        console.log(validator.totalPaginas)
    ...
    //aqui es donde insertamos los productos 
