...
function get_products() {
    return new Promise(resolve => {
        db.getConnection((err, conn) => {
            ...
            query = `
            select p.id_product, sa.quantity from ps_product p
            inner join ps_stock_available sa on sa.id_product = p.id_product
    ...
    console.log(resp_products);
        ...
        if(resp_products['ok'] && resp_products['data']){
            for (key in resp_products['data']) {
                let id_product = '';
                if(resp_products['data'][key]['quantity']<=0){
                    listaToUpdate += `UPDATE ps_product SET active = 0 where id_product = ${resp_products['data'][key]['id_product']}; UPDATE ps_product_shop SET active = 0 where id_product = ${resp_products['data'][key]['id_product']};`
