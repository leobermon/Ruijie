...
function get_products_sin_referencia() {
    return new Promise(resolve => {
        db.getConnection((err, conn) => {
            // obtenemos todos los productos de syscom de la bd novusred
            conn.query("SELECT * FROM ps_product where supplier_reference = '' and id_supplier = 3", function (err, result) {
                if (err) {
...
async function prueba(){
    ...
    const lista_productos_sin_referencia = await get_products_sin_referencia();
